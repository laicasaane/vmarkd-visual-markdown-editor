...
// overlay (`.vmde-stale-overlay`, data-render="1"). SpinVditorIRDOM's `ParseHTML` tokenizes that whole
...
// `patchIrStripPreviewSpin` (ir/input.ts) via `window.__vmdeStripPreviewForSpin`.

/** Return `html` with every `.vditor-ir__preview` render (and `.vmde-stale-overlay`) emptied — a no-op
...
export function stripPreviewForSpin(html: string): string {
  ...
  if (
    ...
    html.indexOf('vmde-stale-overlay') === -1
  )
    return html
  ...
  for (const el of Array.from(
    tpl.content.querySelectorAll('.vditor-ir__preview, .vmde-stale-overlay'),
  )) {
    if (el.firstChild) {
      el.textContent = ''
      stripped = true
