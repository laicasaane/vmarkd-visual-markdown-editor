...
// to the ONE shared main-thread ELK (window.__vmarkdElk), booting it on demand if the caller
...
describe('elk-bundled-shim (drop-in elkjs/lib/elk.bundled.js replacement, task 112)', () => {
  beforeEach(() => {
    vi.clearAllMocks()
    delete (window as any).__vmarkdElk
    delete (window as any).__vmarkdCdn
  })
  ...
  it('delegates straight to the already-booted shared ELK, without booting again', async () => {
    ...
    ;(window as any).__vmarkdElk = { layout }
    const graph = { id: 'root' }
  ...
  it('boots the shared ELK (via window.__vmarkdCdn) when not yet up, then delegates', async () => {
    const layout = vi.fn(() => Promise.resolve('LAID_OUT_AFTER_BOOT'))
    ;(window as any).__vmarkdCdn = 'https://cdn.example'
  ...
  it('falls back to an empty cdn when window.__vmarkdCdn is unset', async () => {
  ...
  it('rejects when the shared engine cannot be booted (caller then falls back to its own default)', async () => {
    ...
    await expect(new ELK().layout({})).rejects.toThrow(
      'vmarkd: shared main-thread ELK unavailable',
    )
  })
})
