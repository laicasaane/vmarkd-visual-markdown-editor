...
// window.__vmarkdD2 for custom-diagrams.ts to read. There is no logic of its own to exercise, but
// the wiring itself IS a real, breakable contract — a renamed export or a forgotten key here is a
// silent runtime failure (custom-diagrams.ts reads window.__vmarkdD2.x as `any`), not a compile
...
describe('d2-entry (lazy D2 bundle bridge, task 165)', () => {
  beforeEach(() => {
    delete (window as any).__vmarkdD2
    vi.resetModules()
  })

  it('exposes exactly the six functions custom-diagrams.ts reads off window.__vmarkdD2', async () => {
    await import('./d2-entry')
    const bridge = (window as any).__vmarkdD2
