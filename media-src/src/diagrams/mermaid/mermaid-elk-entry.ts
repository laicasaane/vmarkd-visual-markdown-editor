...
// mermaid-elk.ts loads it on demand and reads `window.__vmdeMermaidElkLayouts`, then hands the array
...
// delegates to the ONE shared main-thread ELK we already ship for D2 (window.__vmdeElk). The other
...
;(
  window as unknown as { __vmdeMermaidElkLayouts?: unknown }
).__vmdeMermaidElkLayouts = layouts
