...
// `vscode.open` is not guaranteed to land in vMarkd at all for a user who has never explicitly
...
// when BOTH: (a) the SOURCE panel — the one this click came from — is itself a vMarkd webview
...
// vMarkd", exactly what "follow the way you were reading" means), and (b) the TARGET is itself
...
export interface AssetLinkDeps {
  ...
  // 'vmarkd.editor') when the click came from inside a vMarkd webview, plain vscode.open
...
export class AssetLinkActions {
  ...
  private async scrollToFragmentAfterOpen(
    ...
    // of, at `debug` (Output channel "vMarkd", trace level; house style, not console.log), so a
    ...
    // Task 468 — `vscode.openWith` (the local branch above, when the source is vMarkd) is not
    ...
    // absent panel (opened as something other than vMarkd) still resolves to `undefined` once
    ...
    if (!entry) return // opened in something other than a vMarkd webview — nothing to post to
