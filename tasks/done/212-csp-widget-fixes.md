...
**Status:** done — BUG pair · **Impact:** 🔴 high (a) / 🟡 med (b) · **Origin:** task 192 §4; 191 Probes 1 & 3 + §5.6
...
- [x] (a) Image overlay: set `image.isPreview: false` in `buildVditorOptions` (kills the
...
- [x] (b) Copy button: patch `codeRender` (VDITOR_TS_PATCHES registry) to drop the inline
...
- [x] Do NOT weaken the CSP (185/3i already pinned why `unsafe-eval` is the only allowance).

## Verification

- [x] L1: patch-registry mutation test picks up the new patch automatically
  (`patch-mutation.test.ts`); a unit for the copy payload extraction (line numbers/ZWSP
  stripped — see 191 Probe-19 finding).
- [x] L3 real-VS-Code (mandatory — CSP only exists here): (a) dblclick image → no overlay (or
