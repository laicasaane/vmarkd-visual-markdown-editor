...
1. ~~**`patchEchartsThemeInit`'s second `.replace('.setOption(option)', …)` is genuinely silent**~~
   — **fixed by [task 418](../tasks/418-unguarded-echarts-setoption-rewrite.md) (2026-07-28).** It
...
that any of its logic survived. Already tracked as its own item — [task 144](../tasks/144-plantuml-architecture-hardening.md) item 1 — not duplicated here beyond this cross-reference.
