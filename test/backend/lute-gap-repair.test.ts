...
describe('the IR round-trip in a table cell, repaired', () => {
  ...
  // Lute 8928f186 fixes the upstream WYSIWYG builder itself. Keep the local repair in the assertion
  // too: it remains an idempotent compatibility layer for older or re-spun DOM shapes.
  it.each(['a **b**', 'a *b*', 'a [l](u)', 'a $x$', 'a ~~s~~'])(
    'the WYSIWYG builder and compatibility repair preserve: %j',
    (content) => {
      const md = cell(content)
      expect(lute.VditorDOM2Md(lute.Md2VditorDOM(md)).split('\n')[2]).toContain(
        content,
      )
      expect(wysiwygRoundTrip(md).split('\n')[2]).toContain(content)
    },
  ...
  it('preserves two authored spaces before inline code in both modes', () => {
